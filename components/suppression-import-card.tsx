...
export function SuppressionImportCard() {
  ...
  async function importRows() {
    ...
    setPreview(payload);
    setMessage(`Imported ${payload.imported} suppressions. Skipped ${payload.skipped}.`);
  }
  ...
  return (
    <Card className="p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="font-semibold">Import suppressions</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            Load domain or email suppressions in bulk before running discovery or sending messages.
          </p>
        </div>
      ...
      <div className="mt-4 grid gap-4">
        ...
        <Field label="Suppression content">
          <Textarea
            rows={8}
            value={content}
              ...
              <div key={row.rowNumber} className="rounded-md border bg-black/20 p-3 text-xs">
                ...
                {row.row ? (
                  <p className="mt-1 text-muted-foreground">
                    {row.row.email ?? row.row.domain} · {row.row.reason}
                  </p>
                ) : null}
                {row.errors.length > 0 ? (
